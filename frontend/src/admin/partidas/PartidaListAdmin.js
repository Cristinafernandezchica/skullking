...
export default function PartidaListAdmin() {
    ...
    let partidaList;
    if (filtered.length === 0 && (filter !== "" || search !== "")) {
        partidaList =
            <tr>
                <td colSpan="4">No hay consultas con esos filtros y parámetros de búsqueda.</td>
            </tr>
    } else {
        partidaList = partidasAux.map((partida) => {
            const jugadoresList = jugadores[partida.id]?.map((jugador) => (
                <div key={jugador.id}>{jugador.usuario.username}</div>
            )) || <div>Cargando jugadores...</div>;

            const ownerName = owners[partida.ownerPartida]?.username || "Cargando owner...";
            return (
                <tr key={partida.id}>
                    <td>{partida.nombre}</td>
                    <td>{partida.estado}</td>
